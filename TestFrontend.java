...
public class TestFrontend {
    ...
    public void runTests() {
        ...
        //Start of Sherry's tests
        System.out.print("Test genre selection screen functionality (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
        if (this.testFrontendGenreSelection()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
        ...
        System.out.print("Test ratings selection screen functionality (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
        if (this.testFrontendRatingsSelection()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
    ...
    public boolean enterXToExit() {
        PrintStream standardOut = System.out; //simulate console interaction & save
        InputStream standardIn = System.in;  //simulate keystrokes
        try {
            // set the input stream to our input (with an x to test of the program exists)
            String input = "x"; //simulate keystoke "x"
            //convert input into an inputstream
            InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
            System.setIn(inputStreamSimulator);
            //write data into here instead of the console
    ...
    /**
    * This method tests the functionality of selecting movies by genre in genre selection mode.
    *
    * Test succeeds if the correct movies are selected for each desired genre.
    *
    *
    * @return true if test passed, false otherwise.
    */
    public boolean testFrontendGenreSelection() {
        PrintStream standardOut = System.out;
        InputStream standardIn = System.in;
        try {
            // set the input stream to our input
            String input = "r" + System.lineSeparator() + "Horror" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
            InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
            System.setIn(inputStreamSimulator);
            ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
            // set the output to the stream captor to read the output of the front end
    ...
    /**
     * This method tests if passing 'r' as user input will go to the ratings selection mode. It then exits
     * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
     * Much of this code is borrowed from testFrontendGForGenres()
     *
     * Test succeeds if the ratings contain all 3 expected data points.
     * Test fails if the front end is not initialized correctly or if it runs into an exception.
     *
     * @return true if test passed, false otherwise.
     */
    ...
    /**
    * This method tests the functionality of selecting movies by ratings in ratings selection mode.
    *
    * Test succeeds if the correct movies are selected for each desired rating.
    *
    * @return true if test passed, false otherwise.
    */
    public boolean testFrontendRatingsSelection() {
        PrintStream standardOut = System.out;
        InputStream standardIn = System.in;
        try {
            // set the input stream to our input
            ...
            String appOutput = outputStreamCaptor.toString();
            if (frontend != null && appOutput.contains("3.5")) {
                    // hopefully this checks if only the 3.5 movie shows up when rating 3 is selected
                // test passes if all ratings from the data are displayed on the screen
                return true;
            } else {
                // test failed
